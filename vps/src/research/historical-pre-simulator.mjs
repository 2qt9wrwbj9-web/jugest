...
const HISTORICAL_REPLAY_VERSION='historical-shadow-v1';
...
export function initialHistoricalPreState(){
  ...
  return Object.freeze({replayVersion:HISTORICAL_REPLAY_VERSION,model,fingerprint:model.fingerprint,generation:0,seenFingerprints:Object.freeze([model.fingerprint]),champions:freezeRegistry([{fingerprint:model.fingerprint,model,generation:0}]),frontierDate:null});
}
...
export function evolveHistoricalPreState({storeId,historyDays,state=initialHistoricalPreState(),maxSearchRounds=64}={}){
  ...
  if(!split.train.length||!split.validation.length||!split.holdout.length)return Object.freeze({state:Object.freeze({...state,frontierDate}),datasetHash:dataset.inputHash,converged:false,reason:'insufficient_data'});
  ...
  for(let round=0;round<maxSearchRounds;round+=1){
    ...
    if(search.improved&&best?.model&&!repeated&&proposed!==championFingerprint){model=best.model;generation+=1;noImproveCount=0;seen.add(proposed);registry.set(proposed,{fingerprint:proposed,model,generation});continue}
  ...
  if(converged){let winner=null;for(const row of [...registry.values()].sort((a,b)=>a.generation-b.generation||a.fingerprint.localeCompare(b.fingerprint))){const candidate={...row,score:evaluateModel(split.holdout,row.model)};if(holdoutBetter(candidate,winner))winner=candidate}if(winner)model=winner.model}
  const next=Object.freeze({replayVersion:HISTORICAL_REPLAY_VERSION,model,fingerprint:fingerprintModel(model),generation,seenFingerprints:Object.freeze([...seen].sort()),champions:freezeRegistry([...registry.values()].sort((a,b)=>a.generation-b.generation||a.fingerprint.localeCompare(b.fingerprint))),frontierDate});
