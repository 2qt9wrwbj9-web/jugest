...
import {activateStoreModel,getActiveStoreModel,refreshActiveStoreReadSnapshot} from '../research/store-read-output.mjs';
...
async function main(){
  ...
  try{
    ...
    let research=null,storeRead=null;
    if(!completion.job){
      const publishAt=new Date().toISOString();
      research=ensureResearchCycle(db,{storeId,featureVersion,frontierDate:asOfDate,nowIso:publishAt});
      const active=getActiveStoreModel(db,{storeId});
      if(active){
        storeRead=refreshActiveStoreReadSnapshot(db,{storeId,days:eligible,frontierDate:asOfDate,nowIso:publishAt});
      }else{
        const initialized=activateStoreModel(db,{
          storeId,
          fingerprint:research.champion.fingerprint,
          model:research.champion.model,
          featureVersion,
          frontierDate:asOfDate,
          days:eligible,
          nowIso:publishAt,
        });
        storeRead=initialized.snapshot;
      }
    }
    sample();
