...
export function readRuntimeTelemetry(db,{historyLimit=50}={}){
  ...
  return Object.freeze({queue:Object.freeze(counts),queueByState:Object.freeze(queueByState),pendingJobs,recentRuns,latestSchedulerSample:latest?{capturedAt:latest.captured_at,effectiveAvailableBytes:mibToBytes(latest.effective_available_mib),usedRatio:Number(latest.used_ratio)||0,swapUsedBytes:mibToBytes(latest.swap_used_mib),runningChildren:Number(latest.running_children)||0,queueDepth:Number(latest.queue_depth)||0,decision:safeJson(latest.decision_json,{})}:null,memoryProfiles});
}
