...
function incrementQueueState(target,state,n){
  if(state==='retry_wait')target.retryWait+=n;
  else if(Object.hasOwn(target,state))target[state]+=n;
}

export function readRuntimeTelemetry(db,{historyLimit=50}={}){
  const limit=Math.min(100,Math.max(1,Math.trunc(Number(historyLimit)||50))),counts={queued:0,running:0,succeeded:0,failed:0},queueByState=emptyQueueByState();
  ...
  const taskHistory=db.prepare(`SELECT m.id,m.job_id,m.store_id,s.name AS store_name,m.phase,m.task_kind,m.task_version,m.model_fingerprint,
      m.store_machine_count,m.store_size_bucket,m.day_count,m.row_count,m.workload_units,m.started_at,m.ended_at,m.duration_ms,
      m.start_rss_mib,m.end_rss_mib,m.peak_rss_mib,m.cpu_ms,m.status,m.error_class,m.details_json
    FROM analysis_task_metrics m LEFT JOIN stores s ON s.id=m.store_id ORDER BY m.id DESC LIMIT ?`).all(limit).map(row=>Object.freeze({
      metricId:Number(row.id),jobId:row.job_id==null?null:Number(row.job_id),storeId:String(row.store_id||''),storeName:String(row.store_name||row.store_id||''),
      phase:Number(row.phase)||0,taskKind:String(row.task_kind||''),taskVersion:String(row.task_version||''),modelFingerprint:row.model_fingerprint??null,
      storeMachineCount:Number(row.store_machine_count)||0,storeSizeBucket:String(row.store_size_bucket||''),dayCount:Number(row.day_count)||0,rowCount:Number(row.row_count)||0,workloadUnits:Number(row.workload_units)||0,
      startedAt:row.started_at,endedAt:row.ended_at,durationMs:Number(row.duration_ms)||0,startRssBytes:row.start_rss_mib==null?null:mibToBytes(row.start_rss_mib),endRssBytes:row.end_rss_mib==null?null:mibToBytes(row.end_rss_mib),peakRssBytes:row.peak_rss_mib==null?null:mibToBytes(row.peak_rss_mib),cpuMs:row.cpu_ms==null?null:Number(row.cpu_ms),status:String(row.status||''),errorClass:row.error_class??null,details:safeJson(row.details_json,{})
  }));
...
function schedulerHealth(runtime,atIso){
  ...
  const sampleMs=latest?Date.parse(latest.capturedAt||''):NaN;
  const sampleAgeMs=Number.isFinite(nowMs)&&Number.isFinite(sampleMs)?Math.max(0,nowMs-sampleMs):null;
  ...
  const readyWork=by.queued>0||by.leased>0||by.running>0||retryReady;
  const pressure=String(latest?.decision?.pressure||'');
  const staleAfterMs=Math.max(10000,Number(DEFAULT_RESOURCE_POLICY.sampleIntervalMs||2000)*5);
  let code='idle';
  if(by.running>0)code='running';
  else if(by.leased>0)code='leased_not_running';
  else if(!latest&&readyWork)code='no_scheduler_sample';
  else if(sampleAgeMs!==null&&sampleAgeMs>staleAfterMs&&readyWork)code='scheduler_stale';
  else if(pressure==='EMERGENCY')code='memory_emergency';
  else if(pressure==='PAUSE')code='memory_pause';
  else if(by.queued>0||retryReady)code='ready_not_running';
  else if(by.retryWait>0)code='retry_wait';
  return Object.freeze({code,sampleAgeMs,staleAfterMs,readyWork,pressure:pressure||null});
}

export async function buildResourceStatus(db,options={}){
  const snapshot=await captureResourceSnapshot(options),runtime=readRuntimeTelemetry(db,options);
  return Object.freeze({...snapshot,analysis:Object.freeze({...runtime,schedulerHealth:schedulerHealth(runtime,snapshot.timestamp)}),ingest:readIngestTelemetry()});
}
export const __test={mibToBytes,processSnapshot,durationMs,ingestState,schedulerHealth};
