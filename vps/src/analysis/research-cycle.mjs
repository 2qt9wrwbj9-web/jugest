...
export function ensureResearchCycle(db,{storeId,featureVersion,frontierDate,nowIso}={}){
  ...
  if(!existing){
    ...
    db.prepare(`UPDATE research_loops SET feature_version=?,current_fingerprint=?,best_fingerprint=?,no_improve_count=0,repeated_fingerprint=NULL,state='running',last_error=NULL,frontier_date=?,search_round=0,updated_at=? WHERE store_id=?`)
...
export function recordModelSearchCompletion(db,{storeId,frontierDate,championFingerprint,candidateModel=null,improved=false,validationScore=null,scoreJson={},nowIso}={}){
  ...
  if(convergence){
    ...
    return {loop:getResearchLoop(db,{storeId:id}),job:null,stale:false,converged:true,reason:convergence.reason};
  }
