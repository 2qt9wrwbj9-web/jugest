...
import {acquireCollectorRunLease,releaseCollectorRunLease} from './run-lease.mjs';
import {
...
function qualityError(message='parser quality check failed'){
  const error=new Error(message);error.collectorClass='quality';return error;
}
...
export async function runCollectorOnce({
  ...
  const runLeaseMs=Math.max(leaseMs,maxRunMs+60000);
  const acquired=acquireCollectorRunLease(db,{owner,nowIso:startedIso,leaseExpiresIso:plusMs(started,runLeaseMs)});
  if(!acquired){summary.result='already_running';return summary;}
  ...
  try{
    ...
    while(summary.attempted<maxRequests){
      ...
      try{
        ...
        if(day.quality?.grade==='D')throw qualityError(`parser quality D: ${(day.quality.warnings||[]).join(' / ')||'quality check failed'}`);
    ...
    releaseCollectorRunLease(db,{owner,nowIso:asDate(clock()).toISOString()});
  }
}
