...
function authorizedStore(db,storeId,channelId){
  ...
  if(!row)return {status:404,store:null};
  const metadata=safeJson(row.source_metadata_json,{})||{};
  if(String(metadata.collectorChannelId||'')!==channelId)return {status:403,store:null};
  return {status:200,store:{id:row.id,name:row.name,createdAt:row.created_at,updatedAt:row.updated_at}};
}
...
export function createAnalyticsHandler({relayDbPath,canonicalDbPath}={}){
  ...
  return async function analyticsHandler(req,res){
    ...
    if(method!=='GET'&&method!=='HEAD'){
      sendJson(req,res,405,{ok:false,code:'method_not_allowed'},{allow:'GET, HEAD'});
      return;
    }
    const auth=await authenticate(req,relayDbPath);
    if(!auth){sendJson(req,res,401,{ok:false,code:'unauthorized'});return}
    const url=new URL(req.url||'/','http://127.0.0.1');
    const parts=parseApiPath(url.pathname);
    if(!parts||parts[0]!=='api'||parts[1]!=='vps'){
      sendJson(req,res,404,{ok:false,code:'not_found'});return;
    }
    ...
    try{
      ...
      const access=authorizedStore(db,storeId,auth.channelId);
      if(!access.store){sendJson(req,res,access.status,{ok:false,code:access.status===403?'forbidden':'store_not_found'});return}
      ...
      if(parts.length===5&&parts[4]==='days'){
        ...
        const rows=db.prepare(`SELECT d.business_date,d.parser_version,d.quality_status,COUNT(m.machine_key) AS machine_count
          FROM store_days d LEFT JOIN machine_day_data m ON m.store_id=d.store_id AND m.business_date=d.business_date
          WHERE d.store_id=? AND d.quality_status='valid'
          GROUP BY d.store_id,d.business_date ORDER BY d.business_date DESC LIMIT ?`).all(storeId,limit).reverse();
      ...
      if(parts.length===6&&parts[4]==='days'){
        ...
        if(!day){sendJson(req,res,404,{ok:false,code:'day_not_found'});return}
        ...
        sendJson(req,res,200,{ok:true,store:access.store,day:{date:day.business_date,parserVersion:day.parser_version||'',qualityStatus:day.quality_status,machines}});return;
      }

      if(parts.length===6&&parts[4]==='analysis'&&parts[5]==='default'){
        const row=db.prepare(`SELECT business_date,payload_json,payload_hash,updated_at FROM client_snapshots
          WHERE store_id=? AND snapshot_type='store-analysis-default' AND version=?`).get(storeId,ANALYSIS_VERSION);
        sendJson(req,res,200,{ok:true,store:access.store,analysis:row?safeJson(row.payload_json,null):null,businessDate:row?.business_date??null,payloadHash:row?.payload_hash??null,updatedAt:row?.updated_at??null});return;
      }

      if(parts.length===6&&parts[4]==='analysis'&&parts[5]==='history'){
        const rows=db.prepare(`SELECT target_date,component,version,input_hash,output_hash,created_at FROM analysis_receipts
          WHERE store_id=? ORDER BY id DESC LIMIT 100`).all(storeId);
        sendJson(req,res,200,{ok:true,store:access.store,history:rows.map(row=>({targetDate:row.target_date,component:row.component,version:row.version,inputHash:row.input_hash,outputHash:row.output_hash,createdAt:row.created_at}))});return;
      }

      if(parts.length===5&&parts[4]==='status'){
        const row=db.prepare(`SELECT business_date,payload_json,payload_hash,updated_at FROM client_snapshots
          WHERE store_id=? AND snapshot_type='store-latest-status' AND version=?`).get(storeId,ANALYSIS_VERSION);
        const refresh=db.prepare(`SELECT generation,completed_generation,active_job_id,updated_at FROM analysis_refresh_state
          WHERE store_id=? AND analysis_version=?`).get(storeId,ANALYSIS_VERSION);
        ...
        sendJson(req,res,200,{ok:true,store:access.store,status,businessDate:row?.business_date??null,payloadHash:row?.payload_hash??null,updatedAt:row?.updated_at??refresh?.updated_at??null});return;
      }
