...
const helper=sourcePatchTest.STORE_RESET_HELPER;
...
for(const token of ['externalDays=nextDays','modelForecasts=nextForecasts','storeAnalysisHistoryIndex=nextHistory','externalDbSet(externalDays)','storeAnalysisSaveIndex()','autoSaveState()']){
  assert.ok(helper.includes(token),`reset implementation missing ${token}`);
}
assert.ok(!helper.includes('sessions=sessions.filter'), 'Day-1 reset must preserve play-session records');
assert.ok(!helper.includes('shops=shops.filter'), 'Day-1 reset must preserve the store master');
