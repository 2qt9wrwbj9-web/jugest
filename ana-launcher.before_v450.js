...
const aliases=[['my',['マイジャグラーV','マイジャグラー5','マイジャグV','マイジャグ5']],['im',['ネオアイムジャグラーEX','ネオアイムジャグラー','ネオアイム']],['go',['ゴーゴージャグラー3','ゴーゴージャグラーⅢ','ゴージャグ3','ゴージャグⅢ']],['fk',['ファンキージャグラー2','ファンキージャグラーⅡ','ファンキー2','ファンキーⅡ']],['hp',['ハッピージャグラーVⅢ','ハッピージャグラーVIII','ハッピージャグラーV3','ハッピーVⅢ','ハッピーVIII','ハッピーV3']],['gg',['ジャグラーガールズSS','ジャグラーガールズ','ガールズSS']],['mr',['ミスタージャグラー','ミスター']],['um',['ウルトラミラクルジャグラー','ウルトラミラクル','ウルミラ']]];
const machineToken=(name='')=>{let s=String(name);try{s=s.normalize('NFKC')}catch{}return s.toUpperCase().replace(/\s+/g,'').replace(/[‐‑‒–—―]/g,'-').replace(/[・･]/g,'')};
...
function parseHtml(html,date,url,expectedMedian=NaN){
  ...
  const machines=[];let candidateRows=0,invalidRows=0,duplicateRows=0,conflictRows=0,matchedTables=selected.length;const unmatchedJugglerNames=new Map();
  for(const m of selected){
    ...
    for(const row of rr){
      ...
      const key=mi>=0?norm(c[mi]):scope;if(!key){if(mi>=0&&machineToken(c[mi]).includes('ジャグ')){const n=String(c[mi]||'').trim()||'不明';unmatchedJugglerNames.set(n,(unmatchedJugglerNames.get(n)||0)+1)}continue}candidateRows++;
      ...
      machines.push({machine:key,sourceMachineName:mi>=0?c[mi]:(aliases.find(x=>x[0]===key)?.[1]?.[0]||key),tableNo,games,diff,bb:b,rb:r,_explicit:mi>=0});
    }
  }
  ...
  return {date,sourceUrl:url,machines:dedup,quality:{score,grade,warnings,candidateRows,invalidRows,duplicateRows,conflictRows,unmatchedJugglerRows,unmatchedJugglerLabels,diffMissing,matchedTables,machineCounts:counts,totalMachines:dedup.length,explicitTablePreferred:explicitHasTarget}};
}
...
async function fetchDay(date){const url=fetchUrl(date),t0=performance.now();let html='',status=200,statusText='OK',finalUrl=url;
  ...
  const ms=Math.round(performance.now()-t0),expected=median(stored.slice(-30).map(x=>x.machines?.length||0));const day=parseHtml(html,date,finalUrl,expected);if(!day.machines.length){const e=new Error('HTTP 200だが対象8機種の台データを検出できなかった');e.httpStatus=status;e.parseQuality=day.quality;throw e}return{day,ms,httpStatus:status};
}
...
function relayCompactDay(d){return{date:d.date,sourceUrl:d.sourceUrl||'',machines:(d.machines||[]).map(r=>({machine:r.machine,tableNo:r.tableNo,games:+r.games||0,diff:r.diff==null?null:+r.diff,bb:+r.bb||0,rb:+r.rb||0}))}}
...
function sharePreparedJson(){
  ...
  if(navigator.canShare?.({files:[file]})&&navigator.share){
    let p;try{p=navigator.share({files:[file],title:`${shopName} ジャグラーデータ`})}catch(e){p=Promise.reject(e)}
