...
const call=async(origin,body)=>{
  const req=new Request('https://jugest.netlify.app/api/relay',{method:'POST',headers:{'content-type':'application/json','origin':origin},body:JSON.stringify(body)});
  const res=await handler(req,{});return{status:res.status,body:await res.json()};
};
let x=await call('https://jugest.netlify.app',{action:'createPair'});
...
x=await call('https://jugest.netlify.app',{action:'pairStatus',channelId:pair.channelId,receiverToken:pair.receiverToken});
...
x=await call('https://jugest.netlify.app',{action:'peek',channelId:pair.channelId,receiverToken:pair.receiverToken});
if(x.body.count!==1||x.body.next.shop!=='新規テスト店')throw new Error('peek failed');
x=await call('https://jugest.netlify.app',{action:'receive',channelId:pair.channelId,receiverToken:pair.receiverToken});
...
x=await call('https://jugest.netlify.app',{action:'ack',channelId:pair.channelId,receiverToken:pair.receiverToken,messageId});
