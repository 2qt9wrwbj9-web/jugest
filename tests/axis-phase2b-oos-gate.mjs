...
function receipt(i,{store='A',ensembleKey='E1',delta=.01,selectorDecision='SHADOW_CHAMPION',stateAfter='BLOCKED'}={}){
 return{
...
function evaluate({priorReceipts=[],ensembleKey='E1',selectorDecision='SHADOW_CHAMPION',targetDate='2026-04-01',store='A',config}={}){
 return evaluateOperationalGate({store,targetDate,ensembleKey,selectorDecision,priorReceipts,config});
}
...
test('other ensemble and other store evidence is never borrowed',()=>{
 const prior=[
  ...
  receipt(11,{ensembleKey:'E2',stateAfter:'ALLOWED'}),
  receipt(12,{store:'B',stateAfter:'ALLOWED'})
 ];
 const result=evaluate({priorReceipts:prior});
 assert.equal(result.evidenceCount,11);
 assert.equal(result.stateBefore,'BLOCKED');
...
test('blocked ensemble releases after 12 positive exact-ensemble OOS deltas',()=>{
 const prior=Array.from({length:12},(_,i)=>receipt(i,{delta:.01,stateAfter:'BLOCKED'}));
 const result=evaluate({priorReceipts:prior});
...
test('allowed ensemble state is derived from its own latest receipt and can keep below release threshold',()=>{
 const prior=Array.from({length:12},(_,i)=>receipt(i,{delta:.001,stateAfter:i===11?'ALLOWED':'BLOCKED'}));
 const result=evaluate({priorReceipts:prior});
...
test('blocked ensemble can recover when its rolling exact-ensemble evidence improves',()=>{
 const prior=Array.from({length:24},(_,i)=>receipt(i,{delta:.006,stateAfter:'BLOCKED'}));
 const result=evaluate({priorReceipts:prior});
 assert.equal(result.operationalDecision,'SHADOW_CHAMPION');
 assert.equal(result.stateAfter,'ALLOWED');
 assert.equal(result.reason,'oos_gate_released');
...
test('only latest 24 exact-ensemble receipts enter the rolling score',()=>{
 ...
 const result=evaluate({priorReceipts:[...oldBad,...recentGood]});
...
test('selector CONTROL can never be promoted',()=>{
 const prior=Array.from({length:24},(_,i)=>receipt(i,{delta:.1,stateAfter:'ALLOWED'}));
 const result=evaluate({priorReceipts:prior,selectorDecision:'CONTROL'});
 assert.equal(result.operationalDecision,'CONTROL');
...
test('malformed exact-ensemble eligible evidence fails closed instead of being skipped',()=>{
 ...
 const badState=Array.from({length:12},(_,i)=>receipt(i));
 badState[11].gate.stateAfter='MAYBE';
 assert.throws(()=>evaluate({priorReceipts:badState}),/stateAfter|state/i);
});

test('invalid config, dates, or identifiers are rejected deterministically',()=>{
