...
assert.match(pub, /ジャグラー設定判別 v4\.8\.8/);
...
assert.match(readme, /Current app release:\s*\*\*v4\.8\.8\*\*/);
assert.match(readme, /Current bookmarklet loader:\s*`BOOKMARKLET_v4860\.txt`/);

console.log('Netlify production preflight passed: v4.8.8 app + Relay + sync + bridge + Launcher + bookmarklet + CI are Netlify-only');
