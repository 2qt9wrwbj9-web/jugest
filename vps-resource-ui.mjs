...
function fmtInt(value){const n=Number(value);return Number.isFinite(n)?Math.max(0,Math.trunc(n)).toLocaleString('ja-JP'):'—'}
...
<section class="vps-resource-card"><h2>処理実績</h2>${taskHistoryHtml(r)}<p class="vps-resource-note">1店舗1処理単位。店舗台数・台×日件数と、子プロセスのPeak RAM・CPU・処理時間を記録する。Peak RAMはプロセス最大RSSであり、JUGESTだけが専有したRAM量ではない。</p></section>
