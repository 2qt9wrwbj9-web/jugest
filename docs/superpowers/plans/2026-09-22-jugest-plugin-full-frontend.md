...
1. Missing analysis snapshots return `analysis: null` without fabrication or failure.
2. Missing status snapshot with pending refresh state derives `pending` exactly like the existing HTTP API.
3. Analysis history is bounded and contains no raw paths, credentials, or operator-only state.
4. New tools reject cross-channel private stores and respect configured PIA access policy.
5. Existing `judge_machines` output and current-vs-store evidence separation stay unchanged.
...
**Interfaces:**
- Consumes: authorized `storeId`; `client_snapshots` row with `snapshot_type='store-analysis-default'`, version `vps-runtime-v1`.
- Produces: `get_store_analysis({storeId}) -> {ok,store,analysis,businessDate,payloadHash,updatedAt}`.

- [ ] **Step 1: Extend the disposable MCP fixture**
...
```
Return `canonicalDbPath` from `fixture()`:
```js
...
- [ ] **Step 2: Write failing tests for the first new tool only**

Change the expected tool names to:
```js
[
  'judge_machines','list_stores','get_store_days','get_store_day',
  'get_store_analysis','get_store_prediction','get_store_comparison'
]
```
In the OAuth store-tools test add:
```js
...
```
Expected: FAIL because `get_store_analysis` is absent.

- [ ] **Step 4: Implement the first tool**
...
**Interfaces:**
- Consumes: `store-latest-status` snapshot and `analysis_refresh_state`.
- Produces: `get_store_status({storeId}) -> {ok,store,status,businessDate,payloadHash,updatedAt}`.

- [ ] **Step 1: Seed analyzed status state**
...
Change expected tool names to:
```js
[
  'judge_machines','list_stores','get_store_days','get_store_day',
  'get_store_analysis','get_store_status','get_store_prediction','get_store_comparison'
]
```
Add to the OAuth store-tools test:
```js
...
cd vps && JUGEST_PIA_ACCESS_MODE=public node --test tests/mcp-api.test.mjs
```
Expected: FAIL because `get_store_status` is absent.
...
- Produces: `get_store_analysis_history({storeId,limit?}) -> {ok,store,limit,history[]}` with rows `{targetDate,component,version,inputHash,outputHash,createdAt}`.

- [ ] **Step 1: Seed two receipts**
...
- [ ] **Step 2: Write failing history tests and final nine-tool list**
...
cd vps && JUGEST_PIA_ACCESS_MODE=public node --test tests/mcp-api.test.mjs
```
Expected: FAIL because `get_store_analysis_history` is absent.
...
securedTool({
  ...
  inputSchema:{type:'object',additionalProperties:false,properties:{storeId:{type:'string',minLength:1},limit:{type:'integer',minimum:1,maximum:100,default:50}},required:['storeId']},
  annotations:{readOnlyHint:true,openWorldHint:false}
}),
```
Add the branch:
...
**Interfaces:**
- Consumes: final nine-tool MCP surface.
- Produces: regression coverage proving new tools inherit existing auth/store boundaries and do not alter current judgement.

- [ ] **Step 1: Add assistant-read coverage**
...
In the existing legacy Receiver test, which creates a fresh fixture, add:
```js
...
- [ ] **Step 5: Verify configured PIA policy separately**

```bash
cd vps && node --test tests/store-access.test.mjs
```
Expected: owner mode allows only configured owner channels and fails closed; public mode allows only explicit public-native PIA metadata.

The main MCP fixture continues to run with `JUGEST_PIA_ACCESS_MODE=public`; it must not be interpreted as the production owner-only setting.
...
- Produces: Plugin metadata and routing instructions that map user intent to JUGEST tools without embedding JUGEST calculations.
...
```
Keep assertions for the existing App ID, absence of `mcp.json`, screenshot extraction safety, and current-vs-PRE evidence separation. Remove only obsolete assertions that require the old screenshot-only marketing copy.
...
```
Expected: FAIL on version/description/new routing metadata.
...
```json
"shortDescription": "JUGEST analysis for Juggler screenshots, stores, and PRE",
...
- [ ] **Step 4: Rewrite the Skill around intent-to-tool routing**
...
- [ ] **Step 5: Broaden `agents/openai.yaml` while keeping implicit live routing**

Replace it with:
```yaml
interface:
  display_name: "JUGEST Analysis"
  short_description: "Automatically route Juggler screenshots and store analysis to JUGEST"
...
```
If a judgement, PRE model, ranking, calibration, store-share, model-registry, Collector mutation, or deploy-control file appears unexpectedly, stop and review it before continuing.
...
- [ ] **Step 6: Re-run the MCP discovery contract only**

```bash
cd vps && JUGEST_PIA_ACCESS_MODE=public node --test --test-name-pattern="MCP tool list exposes" tests/mcp-api.test.mjs
```
Expected: PASS with exactly nine tools; `judge_machines` uses `noauth`; all store tools advertise OAuth `jugest:read`, `readOnlyHint:true`, `openWorldHint:false`.
