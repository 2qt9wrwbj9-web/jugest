...
const LAUNCHER_SCRIPT_URL=String(document.currentScript?.src||[...document.scripts].reverse().find(s=>/\/ana-launcher\.js(?:[?#]|$)/.test(s.src||''))?.src||'');
const LAUNCHER_ORIGIN=(()=>{try{return LAUNCHER_SCRIPT_URL?new URL(LAUNCHER_SCRIPT_URL,location.href).origin:''}catch{return''}})();
const RELAY_API=`${LAUNCHER_ORIGIN}/api/relay`;
const RELAY_BRIDGE=`${LAUNCHER_ORIGIN}/relay-bridge.html`;
const RELAY_BRIDGE_ORIGIN=LAUNCHER_ORIGIN;
...
if(!LAUNCHER_ORIGIN){alert('JUGEST連携元を認識できなかったよ。ブックマークレットを登録し直してね');return;}
